  ...
  <div class="mt-4 container-fluid">
    <div class="card">
      <div class="card-header">
        <h1 class="text-azul-sena">REGISTRAR CERTIFICADO DOCUMENTO</h1>
        <hr />
        <div>
...
<script lang="ts">
  ...
  export default class Viabilidad extends Vue {
    ...
    registrarViabilidad() {
      for (let i = 0; i < this.docsPadre.length; i++) {
        this.docCertificacion.certificacion_id = this.viabilidadPadre;
        this.docCertificacion.documento_id = this.docsPadre[i].id_documento;

        Axios()
          .post("almacenar_certificacion_documento", this.docCertificacion)
          .then((respuesta) => {
            if (this.docCertificacion.documento_id != '') {
