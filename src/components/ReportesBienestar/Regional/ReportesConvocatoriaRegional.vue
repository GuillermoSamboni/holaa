    ...
    <div class="card">
      <div class="card-header">
        <h1 class="text-azul-sena">
          <h1 class="text-azul-sena">REPORTES CONVOCATORIA REGIONAL</h1>
        </h1>
      </div>
                    ...
                    <div class="row mx-5">
                      ...
                      <div class="col">
                        ...
                        <span class="h2 font-weight-bold mb-0">{{
                          listaConvocatoriasBienestar.length
                        }}</span>
                      </div>
                    </div>
                  </div>
          ...
          <div class="row">
            ...
            <div class="col-lg-10 col-md-12 col-sm-12">
              <paginate-links
                ...
                for="listaConvocatoriasBienestarFiltrado"
                :limit="2"
              ...
              <div class="table-responsive">
                <paginate
                  ref="paginator"
                  name="listaConvocatoriasBienestarFiltrado"
                  :list="listaConvocatoriasBienestarFiltrado"
                  :per="10"
                >
                  <table class="table table-hoverble">
                    <thead id="listado">
                      <tr>
                        <th class="text-nowrap">Nombre Convocatoria</th>
                        <th class="text-nowrap">Regional</th>
                        <th class="text-nowrap">Centro</th>
                        <th class="text-nowrap">Apoyo</th>
                        <th class="text-nowrap">Fase</th>
                      </tr>
                    </thead>
                    <tbody>
                      <template
                        v-if="listaConvocatoriasBienestarFiltrado != null"
                      >
                        <tr
                          v-for="(convocatoria, key) in paginated(
                            'listaConvocatoriasBienestarFiltrado'
                          )"
                          :key="key"
                        >
                          <td>
...
export default class listaConvocatoriasRegional extends Vue {
  canvas = document.getElementsByTagName("canvas");
  verGraficas = false;
  listaRegionales = [];
  listaConvocatoriasBienestar = [];
  listaConvocatoriasBienestarFiltrado = [];
  listaApoyos = [];
  listaCentros = [];
  listaFases = [];
  ...
  paginate = ["listaConvocatoriasBienestarFiltrado"];
  regional = {
    id_regional: null,
    regional: null,
  };
  ...
  mounted() {
    ...
    this.obtenerConvocatoriasBienestar();
    this.obtenerApoyos();
    this.obtenerFases();
  }
  obtenerApoyos() {
  ...
  obtenerConvocatoriasBienestar() {
    Axios()
      .get("listar_vista_Bienestar_Convocatorias")
      .then((respuesta) => {
        if (respuesta.data.status.toLowerCase() == "success") {
          this.listaConvocatoriasBienestar = respuesta.data.results;
          this.listaConvocatoriasBienestarFiltrado = respuesta.data.results;

          let arregloFiltrado = [];

          for (let conv of this.listaConvocatoriasBienestar) {
            ...
            this.listaConvocatoriasBienestar = arregloFiltrado;
            this.listaConvocatoriasBienestarFiltrado = arregloFiltrado;
          }
          if (this.listaConvocatoriasBienestarFiltrado.length == 0) {
            //busco no encontro
            Swal.fire({
            ...
            this.regional = null;
            this.listaConvocatoriasBienestarFiltrado =
              this.listaConvocatoriasBienestar;
          }
  ...
  generarContadoresPorFase() {
    for (let i of this.listaConvocatoriasBienestar) {
  ...
  filtroNacional() {
    this.listaConvocatoriasBienestarFiltrado = this.listaConvocatoriasBienestar;
  }
  exportExcel() {
